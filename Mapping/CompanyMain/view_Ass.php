...
$query = "SELECT student.Student_Id,student.Student_Name,student.Student_Matric,student.Student_Course, sv_mark.time_submit, sv_mark.submit_status FROM student INNER JOIN stud_attachment ON student.Student_Id = stud_attachment.Student_Id INNER JOIN company ON stud_attachment.Company_Id = company.Company_Id INNER JOIN sv_assignment ON sv_assignment.Group_Id = company.Group_Id INNER JOIN supervisor ON sv_assignment.Supervisor_Id = supervisor.Supervisor_Id LEFT JOIN sv_mark ON sv_mark.Student_Id = student.Student_Id  WHERE company.Company_Id =$login_user AND student.semester_session='$session'";
            ...
            <?php
                        ...
                        while (list ($key, $val) = each ($companyArray) ) {
                          ...
                          echo '</td>';
          						...
          						echo '<td>';
								echo '<a class="btn btn-info btn-sm " href="AssementForm.php?Student_Id='.$id.'" '.$disabled2.'>Assessment</a>';
								echo '&nbsp;';
								echo '<a class="btn btn-success btn-sm " href="viewAss.php?Student_Id='.$id.'" '.$disabled3.'>View</a>';
								echo '&nbsp;';
								echo '<a class="btn btn-primary btn-sm " href="updateAss.php?Student_Id='.$id.'" '.$disabled3.'>Update</a>';
