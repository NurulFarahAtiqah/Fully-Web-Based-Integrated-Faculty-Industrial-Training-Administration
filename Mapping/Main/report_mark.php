...
if($status=='Done')
{
$query =  "SELECT student.Student_Id,student.Student_Name,student.Student_Matric,student.Student_Course, report.time_submit, supervisor.Supervisor_Name,supervisor.Supervisor_Id  FROM student INNER JOIN stud_attachment ON student.Student_Id = stud_attachment.Student_Id INNER JOIN company ON stud_attachment.Company_Id = company.Company_Id INNER JOIN sv_assignment ON sv_assignment.Group_Id = company.Group_Id INNER JOIN supervisor ON sv_assignment.Supervisor_Id = supervisor.Supervisor_Id LEFT JOIN report ON report.Student_Id = student.Student_Id  WHERE report.time_submit IS NOT NULL AND student.semester_session='$session'";
$result = mysqli_query($db,$query);
}
else if($status=='Not Done')
{
$query =  "SELECT student.Student_Id,student.Student_Name,student.Student_Matric,student.Student_Course, report.time_submit, supervisor.Supervisor_Name,supervisor.Supervisor_Id FROM student INNER JOIN stud_attachment ON student.Student_Id = stud_attachment.Student_Id INNER JOIN company ON stud_attachment.Company_Id = company.Company_Id INNER JOIN sv_assignment ON sv_assignment.Group_Id = company.Group_Id INNER JOIN supervisor ON sv_assignment.Supervisor_Id = supervisor.Supervisor_Id LEFT JOIN report ON report.Student_Id = student.Student_Id  WHERE report.time_submit IS NULL AND student.semester_session='$session'";
$result = mysqli_query($db,$query);
}
